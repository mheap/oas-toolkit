...
describe("explore-oneof", () => {
  it("returns a renderable comparison for scalar-only unions", async () => {
    const model = await buildOneOfExplorerModel({
      openapi: "3.0.0",
      info: { title: "Scalar API" },
      components: {
        schemas: {
          Identifier: {
            oneOf: [
    ...
    expect(usage.fieldComparison.sharedPaths).toEqual([]);
    expect(usage.fieldComparison.branchViews).toEqual([]);
    expect(usage.fieldComparison.nonSharedPathCount).toBe(0);

    const html = generateOneOfExplorerHtml(model);
    expect(html).toContain("Raw schemas not included in path comparison");
  });
  ...
  it("renders a self contained html explorer with search, layouts, and hash state", () => {
    ...
    expect(html).toContain("De facto default");
    ...
    expect(html).toContain("De facto schema");
    expect(html).toContain("Path schema");
    expect(html).toContain("max-w-6xl");
    expect(html).toContain("max-h-[90vh]");
    expect(html).toContain("lg:grid-cols-2");
    expect(html).toContain("data-open-navigation");
